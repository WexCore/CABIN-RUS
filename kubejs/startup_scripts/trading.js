...
onEvent('item.registry', event => {
	...
	let profession = (name, dname, c1, c2, transactions) => {
		...
		event.create(`profession_card_${id}`)
			...
			.displayName(`Профессия: ${dname}`)
			.unstackable()
	}

	let trade = (name, dname, c1, c2, transactions, custom) => {
		let id = name.toLowerCase().replace("'", "").split(' ').join('_')
		...
		event.create(`trade_card_${id}`)
			...
			.displayName((custom ? "" : "Импорт: ") + dname)
			.unstackable()
	}
	...
	profession("Farming", "Фермер", 0xFFCC29, 0x81B214, [
	...
	profession("Carpentry", "Столяр", 0xD0AF84, 0x966C3B, [
	...
	profession("Mining", "Шахтёр", 0x1C1124, 0x88FFF7, [
	...
	profession("Masonry", "Каменщик", 0x5E6F64, 0xBA7967, [
	...
	profession("Hunting", "Охотник", 0x393E46, 0xCF0000, [
	...
	profession("Cooking", "Повар", 0xD8B384, 0xF7DAD9, [
	...
	let quota = 8
	profession("Fishing", "Рыбак", 0x9DDFD3, 0xDBF6E9, [
	...
	profession("Smithing", "Кузнец", 0xFFC93C, 0xFF7A00, [
	...
	trade("Exchange Currencies", "Обмен валюты", 0xEBA83A, 0xF4F4F4, [
		{ in: G(1), out: S(64) },
		{ in: S(64), out: G(1) }
	], true)
	...
	let simple = (name, dname, item, coin, unit, c1, c2) => {
		if (!simulate)
			trade(name, dname, c1, c2, [{ in: unit(coin), out: item }])
		if (!DATAGEN_QUESTS)
			return
	...
	while (true) {
		...
		entry_cost = 10
		simple("Dirt", "Земля", MC('dirt', 16), 2, S, 0x513825, 0xA87954)
		simple("Sand", "Песок", MC('sand', 16), 4, S, 0xC2B289, 0xD8D6B9)
		simple("Gravel", "Гравий", MC('gravel', 16), 2, S, 0x686160, 0xA19393)
		simple("Clay", "Глина", MC('clay', 16), 6, S, 0x878B95, 0x8E939D)
		simple("Ice", "Лёд", MC('ice', 16), 16, S, 0x7E99CF, 0xABB8D0)
		simple("Blackstone", "Чернит", MC('blackstone', 16), 12, S, 0x140E0F, 0x2D2831)
		simple("Grout", "Grout", TC('grout', 16), 12, S, 0x70737F, 0xAEB0B5)
		simple("Cobblestone", "Булыжник", MC('cobblestone', 16), 1, S, 0x585858, 0x646363)
		simple("Granite", "Гранит", MC('granite', 16), 3, S, 0x563A2F, 0x9A6C5B)
		simple("Diorite", "Диорит", MC('diorite', 16), 3, S, 0x7F7F7F, 0xD4D4D4)
		simple("Andesite", "Андезит", MC('andesite', 16), 3, S, 0x5F5F5F, 0x8E8E8E)
		simple("Deepslate", "Глубинный сланец", MC('deepslate', 16), 12, S, 0x140E0F, 0x2D2831)
		simple("Limestone", "Известняк", CR('limestone', 16), 3, S, 0xA7A89E, 0xC0C2BA)
		simple("Scoria", "Пепел", CR('scoria', 16), 3, S, 0x2A130C, 0x553427)
		simple("Obsidian", "Обсидиан", MC('obsidian', 1), 8, S, 0x05030A, 0x36234C)
		simple("Cobbled Deepslate", "Колотый глубинынй сланец", MC('cobbled_deepslate', 8), 1, S, 0x4A4A4F, 0x737373)
		simple("Honeycomb Block", "Блок пчелинных сот", MC('honeycomb_block', 16), 2, S, 0xFCE17D, 0xE58A08)
		
		next_group()
		entry_cost = 10
		simple("Dead Log", "Мёртвое бревно", BOP('dead_log', 16), 2, S, 0x3D362D, 0x7A756D)
		simple("Oak Log", "Дубовое бревно", MC('oak_log', 16), 4, S, 0x735932, 0xA88756)
		simple("Birch Log", "Берёзовое бревно", MC('birch_log', 16), 4, S, 0xD6D6D2, 0xC4B079)
		simple("Spruce Log", "Еловое бревно", MC('spruce_log', 16), 4, S, 0x523E21, 0x6F522F)
		simple("Jungle Log", "Бревно тропического дерева", MC('jungle_log', 16), 4, S, 0x5A501D, 0x9B6E4C)
		simple("Acacia Log", "Акациевое бревно", MC('acacia_log', 16), 4, S, 0x4F4B42, 0x9E552E)
		simple("Dark Oak Log", "Бревно тёмного дуба", MC('dark_oak_log', 16), 4, S, 0x2C1B0D, 0x422B15)
		simple("Crimson Stem", "Багровый стебель", MC('crimson_stem', 16), 8, S, 0x442332, 0x7A3852)
		simple("Warped Stem", "Искажённый стебель", MC('warped_stem', 16), 8, S, 0x3E1E2D, 0x347776)
		
		next_group()
		entry_cost = 10
		simple("Iron Ingot", "Железный слиток", MC('iron_ingot', 8), 16, S, 0xA6A6A6, 0xD5D5D5)
		simple("Zinc Ingot", "Цинковый слиток", CR('zinc_ingot', 8), 16, S, 0x616A60, 0xD0D2C5)
		simple("Copper Ingot", "Медный слиток", MC('copper_ingot', 8), 16, S, 0xDD7E5D, 0xFCEFBA)
		simple("Nickel Ingot", "Никелевый слиток", TE('nickel_ingot', 8), 32, S, 0x977756, 0xE4D196)
		simple("Lead Ingot", "Свинцовый слиток", TE('lead_ingot', 8), 32, S, 0x232456, 0x7C95A4)
		...
		simple("Invar Ingot", "Инварный слиток", TE('invar_ingot', 4), 64, S, 0x406D6C, 0xC3CAC1)
		
		entry_cost = 10
		simple("Coal", "Уголь", MC('coal', 16), 4, S, 0x1C1C1E, 0x383D45)
		simple("Flint", "Кремень", MC('flint', 16), 4, S, 0x3C3B3B, 0xA6A6A6)
		simple("Cinnabar", "Киноварь", TE('cinnabar', 4), 16, S, 0xFC7781, 0xFCCED0)
		simple("Redstone Dust", "Красная пыль", MC('redstone', 16), 8, S, 0xA80F01, 0xFC7781)
		simple("Diamond", "Алмаз", MC('diamond', 1), 1, G, 0x20C3B3, 0xD2FCF3)
		...
		simple("Amethyst", "Аметист", MC('amethyst_shard', 8), 32, S, 0xBC96AA, 0x523A7E)
		...
		simple("Nether Quartz", "Незер-кварц", MC('quartz', 8), 24, S, 0xB19E8F, 0xE7E2DB)
		simple("Certus Quartz", "Истинный кварц", AE2('certus_quartz_crystal', 8), 24, S, 0x91C5FC, 0xA7CBCF)
		simple("Fluix Quartz", "Изменчивый кристал", AE2('fluix_crystal', 8), 32, S, 0x8F5CCB, 0xFC7ED4)
		simple("Cured Rubber", "Вулканизированная резина", TE('cured_rubber', 6), 16, S, 0x3D363C, 0x594F55)
		simple("Bitumen", "Битум", TE('bitumen', 1), 3, G, 0x0B0909, 0x0B0909)

		
		next_group()
		entry_cost = 10
		simple("Scaffolding", "Строительные леса", MC('scaffolding', 16), 2, S, 0x7F5F41, 0xDDC683)
		simple("Wool", "Шерсть", MC('white_wool', 1), 8, S, 0xBEC4C5, 0xE4E4E4)
		simple("Sponge", "Губка", MC('sponge', 1), 16, S, 0x8F8234, 0xCDCF4A)
		simple("Cobweb", "Паутина", MC('cobweb', 1), 16, S, 0xC2CCCF, 0xFCFCFC)
		
		row += 4
		
		next_group()
		entry_cost = 10
		simple("Rice", "Рис", FD('rice_bag', 1), 4, S, 0x9F7653, 0xCEC6BC)
		simple("Straw", "Солома", FD('straw', 32), 8, S, 0x623A17, 0x966537)
		simple("Glowshroom", "Светящийся гриб", BOP('glowshroom', 4), 9, S, 0x2C65C9, 0x83A7B7)
		simple("Bramble", "Шипастый корень", BOP('bramble', 4), 9, S, 0x8C3122, 0xECCDBC)
		simple("Barley", "Ячмень", BOP('barley', 4), 4, S, 0xB78B44, 0xD8BC64)
		simple("Watergrass", "Водная трава", BOP('watergrass', 4), 4, S, 0x43763D, 0x538B51)
		simple("Reed", "Камыш", BOP('reed', 4), 5, S, 0x7B4E35, 0xB2855C)
		simple("Clover Petal", "Лепесток клевера", BOP('huge_clover_petal', 4), 10, S, 0x5B8A4F, 0x6FA960)
		simple("Spanish Moss", "Испанский мох", BOP('spanish_moss', 4), 7, S, 0x395B2A, 0xA2C790)
		simple("Willow Vine", "Ивовая лоза", BOP('willow_vine', 4), 7, S, 0x265F0D, 0x317B10)
		simple("Cattail", "Рогоз", BOP('cattail', 4), 4, S, 0x186B2B, 0x845738)
		simple("Sugar Cane", "Сахарный тростник", MC('sugar_cane', 4), 3, S, 0x688546, 0xC5FC85)
		simple("Kelp", "Ламинария", MC('kelp', 8), 3, S, 0x5B8131, 0x58A92F)
		simple("Bamboo", "Бамбук", MC('bamboo', 8), 5, S, 0x4F7416, 0x88AC5F)
		simple("Sweet Berries", "Сладкие ягоды", MC('sweet_berries', 8), 11, S, 0x27613F, 0xA30700)
		simple("Glow Berries", "Светящиеся ягоды", MC('glow_berries', 8), 11, S, 0xB7A74F, 0x909223)
		simple("Vines", "Лоза", MC('vine', 4), 7, S, 0x183D08, 0x317B10)
		simple("Glow Lichen", "Светящийся лишайник", MC('glow_lichen', 4), 4, S, 0x515D58, 0x729083)
		simple("Moss Block", "Блок мха", MC('moss_block', 1), 8, S, 0x4D5B23, 0x6F902C)
		simple("Glowing Moss Block", "Блок светящегося мха", BOP('glowing_moss_block', 1), 12, S, 0x144B6A, 0x153044)
		simple("Tree Fertilizer", "Удобрение для деревьев", CR('tree_fertilizer', 1), 8, S, 0xCF8469, 0x71933A)
		
		next_group()
		entry_cost = 10
		simple("Daub", "Мазня", SP('daub', 16), 5, S, 0xBFBAAA, 0xCBC8B6)
		simple("Clear Glass", "Чистое стекло", TC('clear_glass', 16), 4, S, 0xA9C3CF, 0xE8E8E8)
		simple("Copper Shingles", "Медная черепица", CR('copper_shingles', 16), 3, S, 0xB5654B, 0xE4BB79)
		simple("Iron Sheet Metal", "Iron Sheet Metal", CD('iron_sheet_metal', 16), 3, S, 0xE3E3E3, 0x717171)
		...
		simple("Sunmetal", "Солнечный металл", AP('sunmetal_block', 32), 8, S, 0x603E38, 0xB48764)
		simple("Plating Block", "Блок обшивки", AP('plating_block', 32), 8, S, 0x222225, 0x39383C)
		simple("Twisted Planks", "Доски из изогнутого дерева", AP('twisted_planks', 32), 8, S, 0x5E5259, 0x72616B)
		simple("Osseous Bricks", "Костные кирпичи", AP('osseous_bricks', 32), 8, S, 0x9D976F, 0xD3D0BF)
		simple("Seared Stone", "Seared Stone", TC('seared_stone', 16), 32, S, 0x393734, 0x59534F)
		simple("Scorched Stone", "Выжженный камень", TC('scorched_stone', 16), 32, S, 0x382C25, 0x4C3F37)
		
		simple("Lantern", "Фонарь", MC('lantern', 1), 1, S, 0x484F64, 0xF6C765)
		simple("Copper Lantern", "Медный фонарь", SP('copper_lantern', 1), 1, S, 0xB36535, 0xF3B154)
		simple("Brass Lantern", "Латунный фонарь", SP('brass_lantern', 1), 1, S, 0xA47C37, 0xFAEACF)
		simple("Crimson Lantern", "Багровый фонарь", SP('crimson_lantern', 1), 1, S, 0x9C0E2C, 0xE7EB56)
		
		next_group()
		entry_cost = 10
		simple("Slime Ball", "Шарик слизи", MC('slime_ball', 4), 24, S, 0x4F7E48, 0x8AD480)
		simple("String", "Нить", MC('string', 4), 5, S, 0x2E4446, 0xD8D8D8)
		simple("Feather", "Перо", MC('feather', 4), 6, S, 0xD0D0D0, 0xFCFCFC)
		simple("Gunpowder", "Порох", MC('gunpowder', 4), 7, S, 0x535353, 0x717171)
		simple("Leather", "Кожа", MC('leather', 4), 8, S, 0x873A25, 0xC45B34)
		simple("Ink Sac", "Чернильный мешок", MC('ink_sac', 4), 8, S, 0x493F49, 0x786470)
		simple("Glow Ink Sac", "Светящийся чернильный мешок", MC('glow_ink_sac', 2), 8, S, 0x29807E, 0x5AB488)
		simple("Experience", "Опыт", MC('experience_bottle', 1), 8, S, 0x689AC7, 0xFFF2FF)
		simple("Shulker Shell", "Панцирь шалкера", MC('shulker_shell', 1), 2, G, 0x6B476A, 0x956895)
		simple("Spider Eye", "Глаз паука", MC('spider_eye', 4), 10, S, 0x64062A, 0xC25E6A)
		simple("Ender Pearl", "Эндер-жемчуг", MC('ender_pearl', 1), 48, S, 0x0B4C41, 0x2BCBAF)
		simple("Rotten Flesh", "Гнилая плоть", MC('rotten_flesh', 4), 3, S, 0xB24320, 0x695C18)
		simple("Blaze Rod", "Огненный стержень", MC('blaze_rod', 1), 20, S, 0xAC3B00, 0xD5AC26)
		simple("Bone", "Кость", MC('bone', 4), 8, S, 0xC9C4A3, 0xC9C4A3)
		simple("Prismarine Shard", "Осколок призмарина", MC('prismarine_shard', 4), 16, S, 0x2F6355, 0x8FC0AA)
		...
		simple("Frozen Core", "Замороженная сердцевина", RQ('frozen_core', 1), 48, S, 0x008DC2, 0x7571FB)
		simple("Nebulous Heart", "Затуманенное сердце", RQ('nebulous_heart', 1), 3, G, 0x6200A0, 0xE500C3)
		simple("Guardian Spike", "Шип стража", RQ('guardian_spike', 1), 1, G, 0x7F4215, 0xE29964)
		simple("Phantom Membrane", "Мембрана фантома", MC('phantom_membrane', 1), 48, S, 0x6E506B, 0xC1B79F)
		simple("Blaze Cake", "Торт всполоха", CR('blaze_cake', 1), 16, S, 0x834141, 0xFCE083)
		next_group()
